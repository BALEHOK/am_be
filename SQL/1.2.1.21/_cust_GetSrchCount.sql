...
-- =============================================
CREATE PROCEDURE [dbo].[_cust_GetSrchCount]
...
BEGIN
	...
	IF NOT EXISTS (SELECT 1 FROM ##srchcount WHERE SearchId=@SearchId AND UserId=@UserId)
	BEGIN
		INSERT ##srchcount
		SELECT @SearchId, @UserId, 'TotalCount', 0 As Id, COUNT(1) AS Count 
		  FROM ##srchres
		 WHERE SearchId=@SearchId AND UserId=@UserId;
		SELECT @count = COUNT FROM ##srchcount WHERE SearchId = @SearchId AND UserId = @UserId AND [Type] = 'TotalCount';
...
-- Return results
	SELECT * FROM ##srchcount WHERE SearchId=@SearchId AND UserId=@UserId;
END
