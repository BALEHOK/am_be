...
ALTER PROCEDURE [dbo].[_cust_GetColumnNamesByTypeContext]
@TableName varchar(MAX)
AS
BEGIN

	select deac.name, deac.dbtablefieldname, deac.IsShownOnPanel,deac.IsDynListValue, deac.DynListUid, deac.RelatedAssetTypeID, deac.RelatedAssetTypeAttributeID from DynEntityAttribConfig deac
...
-- =============================================
ALTER PROCEDURE [dbo].[_cust_RebuildTriggers]
...
BEGIN
	...
	IF (@rowcount = 0 AND @dynEntityConfigUid > 0)
	BEGIN
		SELECT TOP 1 @tablename = DBTableName FROM DynEntityConfig WHERE DynEntityConfigUid = @dynEntityConfigUid
