/****** Object:  StoredProcedure [dbo].[_cust_SearchCustomQuery]    Script Date: 02/02/2016 19:37:07 ******/
SET ANSI_NULLS ON
GO
					...
					(
						SELECT FT_TBL.IndexUid, FT_TBL.UserId, FT_TBL.OwnerId, FT_TBL.DynEntityConfigId, FT_TBL.DepartmentId, FT_TBL.TaxonomyItemsIds, 
							...
							(
								...
								AND p.ActiveVersion = 1
								AND d.ActiveVersion = 1
								AND pay.DynEntityId NOT IN (SELECT Payment FROM ADynEntitySOBCertificate WHERE ActiveVersion = 1)
								AND (p.District IN (SELECT * FROM f_GetGrantedDistrictIds(@UserId)))
							)										
					) AS SearchResults
			)
...
*/
							...
							(
								...
								AND p.ActiveVersion = 1
								AND d.ActiveVersion = 1
								AND pay.DynEntityId NOT IN (SELECT ISNULL(Payment, 0) FROM ADynEntityBUBCertificate WHERE ActiveVersion = 1)
							)										
					) AS SearchResults
			)
