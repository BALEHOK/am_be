...
namespace AppFramework.CustomResourceProviders
{
    ...
    public class DBResourceProvider : DisposableBaseType, IResourceProvider
    {
        ...
        /// <param name="culture">Language culture of resource</param>
        /// <returns></returns>
        public bool AddResource(string resourceKey, string resourceValue, System.Globalization.CultureInfo culture)
        {
            ResourceRecord rsRec = new ResourceRecord();
            rsRec.ResourceType = this.m_classKey;
            rsRec.ResourceKey = resourceKey;
            rsRec.ResourceValue = resourceValue;
            rsRec.CultureCode = culture.TwoLetterISOLanguageName;

            if (this.m_dalc.AddResource(rsRec)) {
                return true;
            }
            else {
                return false;
