...
namespace AppFramework.CustomResourceProviders
{
    ...
    public class StringResourcesDALC: IDisposable
    {
        ...
        public StringResourcesDALC(string resourceType)
        {
            ...
            this.m_connection = new SqlConnection(ConfigurationManager.ConnectionStrings["CustomResourceProvidersConnectionString"].ConnectionString);
            ...
            m_cmdGetResourceByCultureAndKey.Connection = this.m_connection;
            ...
            m_cmdGetResourcesByCulture.Connection = this.m_connection;
        ...
        /// <summary>
        /// Adds the new resource to the database
        /// </summary>
        /// <param name="culture">Language culture for resource</param>
        /// <param name="resourceKey">Key of resource</param>
        /// <param name="resourceValue">Value of resource</param>
        /// <returns>True if resource was added, false if not</returns>
        public bool AddResource(ResourceRecord resRecord)
        {
            try
            {
                this.m_connection.Open();

                SqlCommand commandAddResource =
                    new SqlCommand("INSERT INTO StringResources " +
                                    "(ResourceType, CultureCode, ResourceKey, ResourceValue) " +
                                    "VALUES (@ResourceType, @CultureCode, @ResourceKey, @ResourceValue)");

                commandAddResource.Connection = this.m_connection;
                commandAddResource.Parameters.AddWithValue("ResourceType", resRecord.ResourceType);
                commandAddResource.Parameters.AddWithValue("CultureCode", resRecord.CultureCode);
                commandAddResource.Parameters.AddWithValue("ResourceKey", resRecord.ResourceKey);
                commandAddResource.Parameters.AddWithValue("ResourceValue", resRecord.ResourceValue);                
                int rows = commandAddResource.ExecuteNonQuery();
                if (rows > 0) {
                    return true;
                }
                else {
