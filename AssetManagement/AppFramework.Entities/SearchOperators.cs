...
namespace AppFramework.Entities
{
    ...
    public partial class SearchOperators: IObjectWithChangeTracker, INotifyPropertyChanged, IDataEntity
    {
        #region Primitive Properties
    
        [DataMember]
        public long SearchOperatorUid
        {
            get { return _searchOperatorUid; }
            set
            {
                if (_searchOperatorUid != value)
                {
                    if (ChangeTracker.ChangeTrackingEnabled && ChangeTracker.State != ObjectState.Added)
                    {
                        throw new InvalidOperationException("The property 'SearchOperatorUid' is part of the object's key and cannot be changed. Changes to key properties can only be made when the object is not being tracked or is in the Added state.");
                    }
                    _searchOperatorUid = value;
                    OnPropertyChanged("SearchOperatorUid");
                }
            }
        }
        private long _searchOperatorUid;
        ...
        private void FixupDataType(object sender, NotifyCollectionChangedEventArgs e)
        {
            ...
            if (e.NewItems != null)
            {
                ...
                {
                    if (!item.SearchOperators.Contains(this))
                    {
                        item.SearchOperators.Add(this);
                    }
                    if (ChangeTracker.ChangeTrackingEnabled)
                    {
            ...
            if (e.OldItems != null)
            {
                foreach (DataType item in e.OldItems)
                {
                    if (item.SearchOperators.Contains(this))
                    {
                        item.SearchOperators.Remove(this);
                    }
                    if (ChangeTracker.ChangeTrackingEnabled)
                    {
